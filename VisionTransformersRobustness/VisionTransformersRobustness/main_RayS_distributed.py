...
runID = datetime.now().strftime('%Y-%m-%d_%H_%M')
...
def net_setup():
	os.environ['MASTER_ADDR'] = '10.250.21.176' #mdc-26-10
...
#SaveLoader(filepath, rank, 'Cifar10_valLoader_{}'.format(args.batch_size), valLoader) 
def SaveLoader(filepath, rank, tag, dataLoader):
    ...
    dataLoaderPathX = '{}/{}_X_{}'.format(filepath, tag, rank)
    dataLoaderPathY = '{}/{}_Y_{}'.format(filepath, tag, rank)
...
def test(local_rank, args):
	...
	rank_print(f"Process for rank: {local_rank} has been spawned")
	rank_print(f'rank {rank}')
	...
	valLoader, defense = LoadViTLAndCIFAR10(None, None, local_test_loader,
													args.validSampleNum, local_rank)
	...
	rank_print(f'Target data inputs {data_inputs.shape}')
	rank_print(f'Target data labels {data_labels.shape}')
	rank_print(f'Number of batches {len(cleanLoader)}')
	rank_print(f'Batch size {cleanLoader.batch_size}')

	cleanAcc, tp_5 = defense.validateD(cleanLoader)
	rank_print(f"Before attack :: Confirmation on target target test dataset: {cleanAcc}\n")
	rank_print(f"Before attack :: Confirmation on Top-5 acc:{tp_5}\n")
